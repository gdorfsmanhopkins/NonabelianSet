...
function moveBalls(column,ballGroup){
  if((column%2)==0){
    steps = column/2;
    ballGroup.translation.set(3*xList[2] + 4*cardWidth*steps/3,activeY);
  }
  else{
    steps = (column-1)/2
    ballGroup.translation.set(5*xList[2] + 4*cardWidth*steps/3,activeY);
  }
}
...
function makeLines(){
  ...
  for(var i=0;i<7;i++){
    nextColumn = []
    for(var j=0;j<3;j++){
      newLine = two.makePath(xList[1],yList[j],xList[2],yList[j],open=true);
      newLine.stroke = 'none';
      newLine.fill = 'none';
      newLine.linewidth = 5;
      newLine.dashes[0] = 50;
...
function moveLines(column,lineGroup){
  lineGroup.translation.set(4*xList[2] + 4*cardWidth*column/3,activeY);
}
